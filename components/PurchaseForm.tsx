...
const PurchaseForm: React.FC<PurchaseFormProps> = ({ 
  ...
  // Cálculo automático de Retenção na Fonte (6,5%) quando o tipo for Serviço
  const totalWithholdingAmount = useMemo(() => {
      return items.filter(it => it.itemType === 'Serviço').reduce((acc, it) => {
          const lineSub = it.quantity * it.unitPrice * (1 - (it.discount || 0) / 100);
          return acc + (lineSub * 0.065);
      }, 0);
  }, [items]);

  const discountGlobalValue = subtotal * (globalDiscount / 100);
  const totalFinal = (subtotal + totalTaxAmount) - discountGlobalValue - totalWithholdingAmount;
  ...
  const handleUpdateItem = (index: number, field: keyof PurchaseItem, value: any) => {
    ...
    if (['quantity', 'unitPrice', 'discount', 'taxRate', 'length', 'width', 'height', 'taxAmount'].includes(field as string)) {
        if (field === 'taxAmount') {
            // Se o IVA for inserido manualmente, recalculamos apenas o total da linha
            const lineSub = item.quantity * item.unitPrice * (1 - (item.discount || 0) / 100);
            item.total = lineSub + Number(value);
        } else {
                ...
                <div className="flex-1 p-0 overflow-x-auto">
                    <table className="w-full text-left border-collapse">
                        <thead className="bg-slate-100 text-slate-500 uppercase text-[10px] font-bold">
                            <tr>
                                <th className="p-3 w-32">Tipo Artigo</th>
                                ...
                                <th className="p-3 w-24 text-right">IVA Valor</th>
                        ...
                        <tbody className="divide-y divide-slate-100">
                            {items.map((item, index) => (
                                <tr key={item.id} className="hover:bg-orange-50/30 transition-colors">
                                    <td className="p-2">
                                        <select 
                                            className="w-full p-1 border border-slate-200 rounded text-[10px] font-black uppercase outline-none focus:ring-1 focus:ring-orange-500" 
                                            value={item.itemType} 
                                    ...
                                    <td className="p-2 text-center">
                                        <select className="w-full p-1.5 border border-slate-200 rounded bg-white text-xs font-bold" value={item.taxRate} onChange={(e) => handleUpdateItem(index, 'taxRate', Number(e.target.value))}>
                                            ...
                                            {taxRates.length === 0 && <><option value={14}>14%</option><option value={7}>7%</option><option value={0}>Isento</option></>}
                                        </select>
                                    </td>
                                    <td className="p-2 text-right">
                                        <input 
                                            type="number" 
                                            className="w-full p-1.5 border border-slate-200 rounded bg-white text-xs font-black text-orange-700 text-right outline-none focus:ring-1 focus:ring-orange-500" 
            ...
            <div className="bg-white rounded-xl shadow-lg border border-slate-200 overflow-hidden sticky top-24">
                ...
                <div className="p-4 space-y-6">
                    ...
                    <div className="space-y-4 pt-2 border-t border-slate-100 text-[10px] uppercase font-bold">
                        ...
                        {/* Visualização de Retenção na Fonte para Serviços */}
                        {totalWithholdingAmount > 0 && (
                            <div className="flex justify-between text-red-600 pt-2 border-t border-slate-50 italic">
                                <span>Retenção na Fonte (6,5%)</span>
                                <span>-{formatCurrency(totalWithholdingAmount)}</span>
                            </div>
                        )}
